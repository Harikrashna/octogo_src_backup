...
namespace CF.Octogo.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : OctogoServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        private string EncryptQueryParametersToBase64(string link, string encrptedParameterName = "Qval")
        {
            if (!link.Contains("?"))
            {
                return link;
            ...
            return basePath + "?" + encrptedParameterName + "=" + System.Convert.ToBase64String(Encoding.UTF8.GetBytes(query));
        }
        ...
        // Desc: Send EmailAddress verification on mail after user SignUp

        public virtual async Task SendUserSignUpEmailActivationLinkAsync(User user, int userTypeId, string UserType, string plainPassword = null)
        {
            await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                await CheckMailSettingsEmptyOrNull();
                ...
                string tenancyName = GetTenancyNameOrNull(user.TenantId); ;
                var link = WebUrlService.GetSiteRootAddress(tenancyName).EnsureEndsWith('/') + "account/confirm-email?userId=" + user.Id.ToString()+"&confirmationCode="+user.EmailConfirmationCode+"&userTypeId="+userTypeId.ToString()+ "&userType="+ UserType;
                link = EncryptQueryParameters(link);
                var emailTemplate = GetTitleAndSubTitle(user.TenantId, L("EmailActivation_Title"), L("EmailActivation_SubTitle"));
                var mailMessage = new StringBuilder();

                mailMessage.AppendLine("<b>" + L("UserType") + "</b>: " + UserType + "<br />");

                mailMessage.AppendLine("<b>" + L("FullName") + "</b>: " + user.Name + " " + user.Surname + "<br />");
