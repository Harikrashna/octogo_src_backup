...
export class CreateOrEditProductComponent extends AppComponentBase {
  ...
  Space(event:any){
      if(event.target.selectionStart ==0 && event.code == 32){
      return false;
      }
  }
  onShown(): void {
  ...
  save(form: NgForm):void{
    ...
    let Duplicacy = this.Product.filter((x) => x.vcProductName.toUpperCase() == this.createProduct.vcProductName.toUpperCase());
